...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-6">
        <a href="{{ route('admin.contact-messages.index') }}" class="text-gray-300 hover:text-white">← Back to Messages</a>
    </div>
    ...
    <!-- Message Details -->
    <div class="bg-neutral-800 shadow rounded-lg p-6 mb-6">
        <div class="flex justify-between items-start mb-4">
            <div>
                <h2 class="text-2xl font-bold text-white">{{ $contactMessage->name }}</h2>
                <p class="text-gray-300">{{ $contactMessage->email }}</p>
                @if($contactMessage->phone)
                    <p class="text-gray-300">{{ $contactMessage->phone }}</p>
                @endif
            </div>
            <div class="text-right">
                ...
                @endif
                <p class="text-sm text-gray-400 mt-2">{{ $contactMessage->created_at->format('M d, Y H:i') }}</p>
            </div>
        </div>
        ...
        <div class="mb-6">
            <form action="{{ route('admin.contact-messages.update-status', $contactMessage) }}" method="POST" class="inline-flex gap-2">
                ...
                <select name="status" onchange="this.form.submit()" class="text-sm rounded border-neutral-600">
        ...
        <div class="border-t pt-4">
            ...
            <div class="text-gray-300 whitespace-pre-wrap">{{ $contactMessage->message }}</div>
        </div>
    ...
    @if($contactMessage->status !== 'replied')
        <div class="bg-neutral-800 shadow rounded-lg p-6">
            <h3 class="text-lg font-bold text-white mb-4">Send Reply</h3>
            
            <form action="{{ route('admin.contact-messages.reply', $contactMessage) }}" method="POST">
                @csrf
                
                <div class="mb-4">
                    <label for="reply_message" class="block text-sm font-medium text-gray-300 mb-2">Reply Message</label>
                    <textarea name="reply_message" id="reply_message" rows="8" required
                        class="block w-full rounded-md border-neutral-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                ...
                <div class="flex justify-end">
                    <button type="submit" class="px-6 py-2 bg-black text-white rounded hover:bg-gray-700">
                        Send Reply
                    </button>
                </div>
            </form>
