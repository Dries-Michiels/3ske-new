...
<div class="max-w-5xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
    <!-- Back Button -->
    <div class="mb-6">
        <a href="{{ route('shows.index') }}" class="text-gray-300 hover:text-white">
            ← Back to Shows
        </a>
    </div>

    <!-- Event Header -->
    <div class="bg-neutral-800 rounded-lg shadow-lg overflow-hidden border border-neutral-700">
        @if($event->image_path)
        ...
        <div class="p-8">
            ...
            <div class="mb-4">
                <div class="flex items-start justify-between">
                    ...
                    @else
                        <span class="px-3 py-1 bg-neutral-700 text-gray-300 text-sm font-semibold rounded-full">
                            Past Event
                        </span>
                    @endif
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach($event->tags as $tag)
                            <a href="{{ route('shows.index', ['tag' => $tag->slug]) }}" class="px-3 py-1 bg-neutral-700 text-gray-200 text-sm rounded hover:bg-neutral-600 border border-neutral-600">
                                {{ $tag->name }}
                            </a>
                        @endforeach
                    </div>
            ...
            @if($event->ticket_url && $event->isUpcoming())
                <div class="mb-6">
                    <a href="{{ $event->ticket_url }}" target="_blank" class="inline-block px-6 py-3 bg-white text-black font-semibold rounded-lg hover:bg-gray-200 transition-colors">
                        🎟️ Get Tickets
                    </a>
                </div>
            @endif
